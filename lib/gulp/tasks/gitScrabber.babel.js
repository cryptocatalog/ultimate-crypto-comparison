...
function getUrlOfLibrary(library) {
    let url = null;
    if (libraryHasDownloadUrl(library)) {   // Download link specified -> use the download link
        url = library[DOWNLOAD_KEY].childs[0][0][0].content;
...
function libraryHasDownloadUrl(library) {
    return !library.tag.startsWith("Template") && library[DOWNLOAD_KEY] && library[DOWNLOAD_KEY].childs[0][0];
}
