...
export function cveInfo(files, done) {
    ...
    function getAllSummariesOfApiResponse(apiResponse){
        let summaryString = "";
        let counter = 0;
        while (apiResponse[counter]){
            ...
            summaryString = summaryString === "" ? "- " + apiString : summaryString + "\\n" + " - " + apiString + "";
            counter++;
        }
    ...
    function getCveDetailsFromApi(){
        ...
        dataJSON.forEach(library => {
            ...
            if (vendor && product) {
                ...
                let cleanSummary = getAllSummariesOfApiResponse(jsonFromApi);
