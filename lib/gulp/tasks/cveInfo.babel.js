...
export function cveInfo(files, done) {
    ...
    function getAllSummariesOfApiResponse(apiResponse, vendor, product) {
        ...
        while (apiResponse[counter]) {
            ...
            apiString = replaceAll(apiString, '"', '\"');
            apiString = replaceAll(apiString, '\0', '\\0');
            // Link to cve entry
            ...
            let cvssAPIString = apiResponse[counter].cvss;
            let cvssColor = determineScoreColor(cvssAPIString);
            let cvssString = '\<span style=\\"color:' + cvssColor + '\\"\>Score: ' + cvssAPIString + '\<\/span\>';

            let newSummaryEntry = "- " + cvssString + ": " + apiString + " " + cveLink;

            // Make a list out of the summaries
            summaryString = summaryString === "" ? newSummaryEntry :
                summaryString + "\\n" + newSummaryEntry;
    ...
    function determineScoreColor(cvss) {
        // 0 <= cvss <= 10
        if (cvss <= 1) {
            return "#00c400";
        } else if (cvss <= 2) {
    ...
    function addSummaryToDataJSON(library, cleanSummary) {
        ...
        try {
            ...
            console.log("ERR in library " + library.tag);
            console.log(err.message);
            // Add error text to the section so the user does not see an empty section
            let errText = "There was an error collecting the CVE details";
            addSummaryToDataJSON(library, errText);
        }
    }
