...
import {gitScrabber} from "./tasks/gitScrabber.babel";
import {cveInfo} from "./tasks/cveInfo.babel";
...
function runGitScrabber(done) {
    gitScrabber(files, done);
}

function getCVEInfos(done) {
    cveInfo(files, done);
}
...
// DEFAULT task --------------------------------------------------<
gulp.task('default', gulp.series(markdown, runGitScrabber, getCVEInfos, changeCriteria, colors, changeCitation, 'development-column', assets));
// --------------------------------------------------------------->
